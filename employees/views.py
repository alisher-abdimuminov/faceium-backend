...
@decorators.api_view(http_method_names=["GET"])
@decorators.permission_classes(permission_classes=[permissions.IsAuthenticated])
def employee_vocations(request: HttpRequest, uuid: str):
...
@decorators.api_view(http_method_names=["POST"])
@decorators.permission_classes(permission_classes=[permissions.IsAuthenticated])
def add_vocation(request: HttpRequest, uuid: str):
    employee = Employee.objects.filter(uuid=uuid).first()
    type = request.data.get("type")
    print(request.data.get("start"))
    start = datetime.strptime(request.data.get("start"), "%Y-%m-%d")
    end = datetime.strptime(request.data.get("end"), "%Y-%m-%d")
